...
function resizeImage(dataUrl, maxW) {
  ...
  return new Promise(function(res) {
    ...
    img.onload = function() {
      ...
      // Adaptive quality — target under 350KB per photo
      var quality = 0.72;
...
function App(){
  ...
  var us=useState(SCREEN.INSPECT);
  ...
  var uexp=useState(0);               var tokenExpiry=uexp[0], setTokenExpiry=uexp[1];
  ...
  // Get fresh token at submit time - shows account picker if needed
    ...
    return new Promise(function(resolve) {
      ...
      try {
        var tc = window.google.accounts.oauth2.initTokenClient({
          client_id : CLIENT_ID,
          scope     : "https://www.googleapis.com/auth/gmail.send email profile",
          hint      : gmailUser ? gmailUser.email : "",
          callback  : async function(resp) {
            if (resp.error) { resolve(null); return; }
            // Also get/update user profile
            try {
  ...
  // Silent token refresh - no popup shown to user
  ...
  // Check and refresh token before any API call
  async function ensureValidToken() {
    // If token expired or expiring in next 2 minutes
    if (!accessToken || Date.now() > tokenExpiry - 2 * 60 * 1000) {
      setSendStatus("Refreshing session...");
      var refreshed = await refreshToken();
      if (!refreshed) {
        // Silent refresh failed - show sign in
  ...
  function signOut(){
    ...
    setScreen(SCREEN.INSPECT); // Stay on inspect, sign in at next submit
  }
  ...
  var canSubmit=requiredDone&&vehicle.reg.trim()&&office&&office.email;

  async function handleSubmit(){
    setScreen(SCREEN.SENDING); setError("");
    try{
      // Sign in fresh at submit time
      setSendStatus("Signing in...");
  ...
  return React.createElement("div",{style:S.page},
    ...
    React.createElement("div",{style:S.header},
      React.createElement("div",{style:S.headerRow},
        ...
        gmailUser&&React.createElement("div",{style:S.userChip,onClick:signOut},
          gmailUser.picture
            ?React.createElement("img",{src:gmailUser.picture,alt:"",style:S.avatar})
            :React.createElement("div",{style:S.avatarFb},gmailUser.name?gmailUser.name[0]:"?"),
          React.createElement("span",{style:S.uname},gmailUser.name?gmailUser.name.split(" ")[0]:"")
        )
      ),
