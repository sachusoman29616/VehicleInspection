...
// Email sent via Brevo API — No login needed
...
var OFFICE_CODES = {
  "101783": {
    ...
    apiKey: "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"
  },
};
...
function validateOfficeCode(code) {
  ...
  if(found) return {success:true,officeName:found.name,officeEmail:found.email,officeApiKey:found.apiKey,officeCode:foundKey};
...
async function sendViaBrevo(apiKey, to, subject, bodyText, pdfBase64, pdfFilename) {
  ...
  var resp = await fetch(BREVO_API_URL, {
    method : "POST",
    headers: {
  ...
  if(!resp.ok) throw new Error(result.message||"Failed to send email");
  return result;
}
...
function App(){
  ...
  function handleValidateOffice(){
    ...
    if(result.success){
      var officeData={code:result.officeCode,name:result.officeName,email:result.officeEmail,apiKey:result.officeApiKey};
  ...
  async function handleSubmit(){
    ...
    try{
      ...
      await sendViaBrevo(office.apiKey, office.email, subject, bodyText, pdfBase64, pdfFilename);
