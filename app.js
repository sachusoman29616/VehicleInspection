...
function stampImage(dataUrl, label, timestamp, gps, placeName) {
  return new Promise(function(res) {
    ...
    img.onload = function() {
      ...
      var pad = 10;
      var locationLine = placeName ? ("[GPS] "+placeName) : (gps ? ("GPS: "+gps.lat+", "+gps.lng) : "GPS: Unavailable");
...
function generateQRDataURL(text) {
  try {
    // Try qrcode-generator first (window.qrcode)
    if (window.qrcode) {
      var qr = window.qrcode(0, "L");
      qr.addData(text.substring(0, 300)); // limit text length
      qr.make();
...
async function generateQR(text) {
  ...
  // Fallback: fetch QR from Google Charts API
  try {
    var shortText = encodeURIComponent(text.substring(0, 200));
    var url  = "https://chart.googleapis.com/chart?chs=200x200&cht=qr&chl="+shortText+"&choe=UTF-8";
...
async function buildPDF(opts) {
  ...
  var reportId=opts.reportId, gps=opts.gps, placeName=opts.placeName, qrDataUrl=opts.qrDataUrl;
  ...
  function coverPage() {
    ...
    // QR code on cover
    if (qrDataUrl) {
      try { doc.addImage(qrDataUrl,"PNG",W-margin-30,54,28,28); } catch(e){}
      doc.setFontSize(6); doc.setFont("helvetica","normal"); doc.setTextColor(191,219,254);
      doc.text("Scan to verify",W-margin-26,84);
    }

    // Vehicle details
    var bx=margin,by=70,bw=W-margin*2-(qrDataUrl?32:0),bh=56;
    ...
    // Inspector box
    var iy=by+bh+6;
    doc.setFillColor(239,246,255); doc.roundedRect(margin,iy,W-margin*2,22,4,4,"F");
    doc.setFontSize(7); doc.setFont("helvetica","bold"); doc.setTextColor(100,116,139);
    doc.text("INSPECTOR",margin+6,iy+8);
    doc.setFontSize(9); doc.setFont("helvetica","bold"); doc.setTextColor(15,23,42);
    doc.text(inspectorName,margin+6,iy+15);
    doc.setFontSize(8); doc.setFont("helvetica","normal"); doc.setTextColor(71,85,105);
    doc.text(inspectorEmail, margin+6+doc.getTextWidth(inspectorName)+4, iy+15);

    // GPS / Location box
    var ly=iy+28;
    doc.setFillColor(240,253,244); doc.roundedRect(margin,ly,W-margin*2,22,4,4,"F");
    doc.setDrawColor(134,239,172); doc.roundedRect(margin,ly,W-margin*2,22,4,4,"S");
    doc.setFontSize(7); doc.setFont("helvetica","bold"); doc.setTextColor(21,128,61);
    doc.text("GPS LOCATION",margin+6,ly+8);
    doc.setFontSize(9); doc.setFont("helvetica","bold"); doc.setTextColor(15,23,42);
    doc.text(placeName||"Location not available",margin+6,ly+15);
    doc.setFontSize(7); doc.setFont("helvetica","normal"); doc.setTextColor(100,116,139);
    var coordText = gps ? ("Coordinates: "+gps.lat+", "+gps.lng+" (Accuracy: ±"+gps.accuracy+"m)") : "GPS coordinates not available";
    doc.text(coordText,margin+6,ly+20);
    ...
    PHOTO_SLOTS.forEach(function(slot,i){
      ...
      if(captured && photos[slot.id].placeName){
        ...
        doc.text("GPS: "+photos[slot.id].placeName, margin+110, ry+5.5);
      }
    });
  ...
  for(var i=0; i<capturedSlots.length; i++){
    ...
    await new Promise(function(resolve){
      ...
      tmp.onload=function(){
        ...
        var ph_h=Math.min(pw*ar, H-py-52);
        try{ doc.addImage(ph.url,"JPEG",px,py,pw,ph_h); }catch(e){}

        // Info strip below photo
        var sy=py+ph_h+4;
        doc.setFillColor(248,250,252); doc.roundedRect(px,sy,pw,28,3,3,"F");
        doc.setDrawColor(226,232,240); doc.roundedRect(px,sy,pw,28,3,3,"S");
        ...
        doc.text(locText, px+5, sy+27);

        // Photo number
        doc.setFontSize(7); doc.setFont("helvetica","normal"); doc.setTextColor(100,116,139);
        doc.text("Photo "+(i+1)+" of "+capturedSlots.length, W-margin-20, sy+14);

        // QR on each photo page (small)
        if(qrDataUrl){
          try{ doc.addImage(qrDataUrl,"PNG",W-margin-18,sy+2,16,16); }catch(e){}
        }
        resolve();
      };
      tmp.src=ph.url;
...
function buildMimeEmail(opts){
  var from=opts.from,to=opts.to,subject=opts.subject,bodyText=opts.bodyText,pdfBase64=opts.pdfBase64,pdfFilename=opts.pdfFilename;
...
function App(){
  ...
  var ue=useState(localStorage.getItem("ui_recipient")||""); var toEmail=ue[0],setToEmail=ue[1];
  ...
  useEffect(function(){
    ...
    if(!window.QRCode){
      ...
      q.src="https://cdnjs.cloudflare.com/ajax/libs/qrcodejs/1.0.0/qrcode.min.js";
  ...
  var capturePhoto=useCallback(async function(slotId,file){
    ...
    reader.onload=async function(e){
      ...
      setPhotos(function(p){
        ...
        n[slotId]={url:stamped,ts:ts,gps:photoGps,placeName:photoPlace};
        return n;
      });
  ...
  async function handleSubmit(){
    ...
    try{
      ...
      // Build QR content — all key report details
      ...
      var doc=await buildPDF({
        ...
        gps:currentGps, placeName:currentPlace, qrDataUrl:qrDataUrl
      });
      ...
      PHOTO_SLOTS.forEach(function(s){
        var line=photos[s.id]
          ? ("✓  "+s.label+"  "+formatTs(photos[s.id].ts)+(photos[s.id].placeName?"  📍 "+photos[s.id].placeName:""))
          : ("—  "+s.label+"  Not captured");
        bodyLines.push(line);
      });
      bodyLines.push("","QR CODE: Scan the QR on the PDF to verify this report","PDF attached: "+pdfFilename,"","---","Sent via UniInspect");
      ...
      React.createElement("div",{style:S.centerCard},
        ...
        React.createElement("div",{style:S.featureBox},
          ["📍 GPS with actual place name on every photo",
           ...
           "⏱️ Timestamp on every photo",
           "✉️ Inspector Gmail identity",
           "🆔 Unique Report ID"].map(function(f){
  ...
  if(screen===SCREEN.DONE) return React.createElement("div",{style:S.page},
    React.createElement("div",{style:S.center},
      React.createElement("div",{style:S.centerCard},
        ...
        placeName&&React.createElement("div",{style:S.locationBox},
          React.createElement("span",null,"[GPS] "+placeName)
        ),
        ...
        React.createElement("p",{style:{fontSize:12,color:"#64748b",marginBottom:20}},"PDF with QR code downloaded to your device."),
  ...
  return React.createElement("div",{style:S.page},
    React.createElement("div",{style:S.header},
      ...
      React.createElement("div",{style:S.fromBadge},
        React.createElement("span",null,"From: "+(gmailUser?gmailUser.email:"")),
        placeName&&React.createElement("span",{style:{color:"#86efac"}},"[GPS] "+placeName)
      )
    ),
    ...
    React.createElement("div",{style:S.body},
      ...
      React.createElement("div",{style:S.card},
        ...
        React.createElement("p",{style:S.hint},"Each photo is stamped with label, timestamp and actual place name automatically."),
              ...
              cap
                ?React.createElement(React.Fragment,null,
                    ...
                    React.createElement("div",{style:S.photoBar},
                      ...
                      cap.placeName&&React.createElement("span",{style:{fontSize:9,color:"#22c55e"}},"📍"),
      ...
      requiredDone&&React.createElement("div",{style:S.securityCard},
        ...
        [
          ["✓","Inspector Gmail: "+(gmailUser?gmailUser.email:""),true],
          ["✓","Unique Report ID will be generated",true],
          [gps?"✓":"!","GPS: "+(placeName?("[GPS] "+placeName):(gps?(gps.lat+", "+gps.lng):"Will capture on submit")),!!gps],
          ["✓","QR code will be added to PDF",true],
          ["✓","Timestamp on every photo",true],
        ].map(function(row,i){
