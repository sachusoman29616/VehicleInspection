...
function stampImage(dataUrl,label,timestamp,gps,placeName,verified,sentTime) {
  return new Promise(function(res) {
    ...
    img.onload=function(){
      ...
      var fh=Math.max(13,Math.round(img.height*0.026)),pad=10;
      var locationLine=placeName?placeName:(gps?("GPS: "+gps.lat+", "+gps.lng):"GPS: Unavailable");
      var lines=[];
      lines.push(label.toUpperCase());
      if(verified){
        // Internet time verified - single clean timestamp
        lines.push("🕐 "+timestamp+" ✓");
      } else {
        // Phone time - show device time + note
        lines.push("📱 "+timestamp+" (Device)");
        lines.push("   Internet unavailable at capture");
        if(sentTime){
          lines.push("🌐 "+sentTime+" (Sent)");
        }
      }
      ...
      var colors=[];
      if(verified){
        colors=["#ffffff","#fbbf24","#86efac"];
      } else {
        colors=["#ffffff","#fbbf24","#94a3b8",sentTime?"#60a5fa":"#94a3b8","#86efac"];
      }
...
function App(){
  ...
  useEffect(function(){
    var interval=setInterval(async function(){
      ...
      expired.forEach(function(r){
        removeFromQueue(r.reportId);
      });
      if(expired.length>0) refreshQueue();
  ...
  async function captureFromCamera(){
    ...
    closeCamera();
    var slot=PHOTO_SLOTS.find(function(s){return s.id===slotId;});
    // Try internet time first, fallback to phone time
    var timeResult = await getTrueTime();
    var ts         = timeResult.verified ? timeResult.date : new Date();
    var tsVerified = timeResult.verified;
    setPhotos(function(p){
      var n=Object.assign({},p);
      n[slotId]={url:dataUrl,ts:ts,gps:null,placeName:null,processing:true,verified:tsVerified};
      return n;
    });
    ...
    var stamped=await stampImage(resized,slot.label,formatTs(ts),photoGps,photoPlace,tsVerified,null);
