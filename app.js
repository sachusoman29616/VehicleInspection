...
const APPS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbwOQZMqfmWbEhG6PhsiDA40SWiaNLvWOdqVQVRQllaG7K7yI7GlJBDMMVJE8tuxfnsaFw/exec";

// Office codes hardcoded — no server needed for validation
var OFFICE_CODES = {
...
function validateOfficeCode(code) {
  ...
  Object.keys(OFFICE_CODES).forEach(function(key) {
    if (key.toUpperCase() === trimmed) found = OFFICE_CODES[key];
  });
  if (found) {
    return { success:true, officeName:found.name, officeCode:code.trim().toUpperCase() };
  }
...
async function submitToServer(payload) {
  try {
    // Add recipient email from local office codes
    var officeKey = payload.officeCode ? payload.officeCode.toUpperCase() : "";
    var officeInfo = OFFICE_CODES[officeKey] || null;
    if (officeInfo) payload.recipientEmail = officeInfo.email;

    payload.action = "submitReport";
    // Use no-cors — request goes through, we assume success
...
function App(){
  ...
  async function handleValidateOffice(){
    ...
    setError("");
    var result=validateOfficeCode(officeCode.trim().toUpperCase());
    if(result.success){
