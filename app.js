...
// No OAuth — Email sent via Apps Script

const { useState, useRef, useCallback, useEffect } = React;

const APPS_SCRIPT_URL = "https://script.google.com/macros/s/AKfycbzHkbt7x0T5bLNNFCNjODweuImNkbUr-_SN7LA0v3y21eu4Lg6GGbauao0O2c5E2nwFKA/exec";
...
function getDeviceFingerprint() {
  try {
    return btoa([navigator.userAgent,screen.width+"x"+screen.height,navigator.language,new Date().getTimezoneOffset()].join("|")).substring(0,20);
...
function getGPS() {
  return new Promise(function(resolve) {
    ...
    navigator.geolocation.getCurrentPosition(
      function(pos) { resolve({ lat:pos.coords.latitude.toFixed(6), lng:pos.coords.longitude.toFixed(6), accuracy:Math.round(pos.coords.accuracy) }); },
...
async function validateOfficeCode(code) {
  try {
    var resp = await fetch(APPS_SCRIPT_URL+"?action=validateOffice&code="+encodeURIComponent(code));
    return await resp.json();
  } catch(e) { return {success:false,error:"Cannot connect to server. Check internet connection."}; }
}

async function submitToServer(payload) {
  try {
    payload.action = "submitReport";
    var resp = await fetch(APPS_SCRIPT_URL,{method:"POST",body:JSON.stringify(payload)});
    return await resp.json();
...
async function buildPDF(opts) {
  ...
  if(gps){
    ...
    var coordText="("+gps.lat+", "+gps.lng+" +/-"+gps.accuracy+"m)";
    doc.text(coordText,margin+6+doc.getTextWidth(placeName||"Not available")+2,ly+14);
  }
...
var SCREEN={INSPECT:"inspect",SENDING:"sending",DONE:"done"};
...
function App(){
  ...
  async function handleValidateOffice(){
    ...
    setValidating(false);
    if(result.success){
      var officeData={code:result.officeCode,name:result.officeName};
  ...
  async function openCamera(slotId){
    setActiveSlot(slotId); setCamOpen(true);
  ...
  async function startCamera(facing){
    ...
    try {
      var stream=await navigator.mediaDevices.getUserMedia({video:{facingMode:{ideal:facing},width:{ideal:1920},height:{ideal:1080}}});
      ...
      try{
        ...
        if(videoRef.current){videoRef.current.srcObject=stream2;videoRef.current.play();}
      }catch(e2){alert("Camera not available. Please allow camera access.");setCamOpen(false);}
    }
  }

  function switchCamera(){startCamera(camFacing==="environment"?"user":"environment");}
  ...
  var capturePhoto=useCallback(async function(slotId,file){
    ...
    reader.onload=async function(e){
      setPhotos(function(p){var n=Object.assign({},p);n[slotId]={url:e.target.result,ts:ts,gps:null,placeName:null,processing:true};return n;});
  ...
  async function handleSubmit(){
    ...
    try{
      ...
      var qrContent=["UNINSPECT VERIFICATION","Report ID: "+reportId,"Office: "+(office?office.name:"—"),"Submitted: "+generatedAt,"Registration: "+vehicle.reg,"Location: "+(currentPlace||"—"),"GPS: "+(currentGps?(currentGps.lat+", "+currentGps.lng):"—")].join("\n");
      ...
      var result=await submitToServer({
        reportId:reportId,submittedAt:generatedAt,
        inspectorName:"Inspector",inspectorEmail:"",
        officeCode:office?office.code:"—",officeName:office?office.name:"—",
        registration:vehicle.reg,makeModel:vehicle.make||"—",
        owner:vehicle.owner||"—",expiredPolicy:vehicle.policy||"—",
        gpsLat:currentGps?currentGps.lat:"—",gpsLng:currentGps?currentGps.lng:"—",
        gpsAccuracy:currentGps?currentGps.accuracy:"—",placeName:currentPlace||"—",
        photosCaptured:PHOTO_SLOTS.filter(function(s){return photos[s.id];}).map(function(s){return s.label;}).join(", "),
        deviceFingerprint:deviceFp,pdfBase64:pdfBase64,pdfFilename:pdfFilename,
      });

      if(!result||!result.success) throw new Error(result?result.error:"Failed to send report");
          ...
          :React.createElement("div",null,
              ...
              React.createElement("div",{style:{display:"flex",gap:8}},
                React.createElement("input",{
                  ...
                  placeholder:"e.g. OFFICE001",value:officeCode,
                ...
                React.createElement("button",{
                  ...
                  onClick:handleValidateOffice,disabled:validating||!officeCode.trim()
                },"Confirm")
              ),
