...
// Fixed sender: uninspect.sender@gmail.com
...
// Office codes — add more offices here
var OFFICE_CODES = {
...
function validateOfficeCode(code) {
  ...
  if(found) return {success:true,officeName:found.name,officeEmail:found.email,officeCode:code.trim().toUpperCase()};
  return {success:false,error:"Please contact your Parent office for office code mapping."};
}

// Get fresh Gmail token for uninspect.sender@gmail.com
...
async function sendViaGmail(token, to, subject, body, pdfBase64, pdfFilename) {
  var boundary = "uninspect_boundary_"+Date.now();
  var emailLines = [
    "MIME-Version: 1.0",
    "To: "+to,
    "From: UniInspect <"+SENDER_EMAIL+">",
    "Subject: "+subject,
...
// Save to Apps Script (no CORS issue for logging only - fire and forget)
function saveToSheets(payload) {
  try {
    payload.action="submitReport";
    fetch(APPS_SCRIPT_URL,{method:"POST",body:JSON.stringify(payload),mode:"no-cors"});
  } catch(e){}
}
...
function App(){
  ...
  async function handleSubmit(){
    ...
    try{
      ...
      setSendStatus("Signing in to send...");
      ...
      await sendViaGmail(token,to,subject,body,pdfBase64,pdfFilename);

      // Save to sheets silently
      saveToSheets({
