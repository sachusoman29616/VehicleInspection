...
async function buildPDF({ vehicle, photos, inspectorName, inspectorEmail, generatedAt }) {
  ...
  const iy=by+bh+8;
  doc.setFillColor(239,246,255); doc.roundedRect(bx,iy,bw,26,4,4,"F");
  doc.setFontSize(7); doc.setFont("helvetica","bold"); doc.setTextColor(100,116,139);
  doc.text("INSPECTOR", bx+6, iy+8);
  doc.setFontSize(9); doc.setFont("helvetica","bold"); doc.setTextColor(15,23,42);
  doc.text(inspectorName, bx+6, iy+17);
  doc.setFontSize(8); doc.setFont("helvetica","normal"); doc.setTextColor(71,85,105);
  doc.text(inspectorEmail, bx+6, iy+23);
  ...
  doc.text("UniInspect — Vehicle Inspection Report", margin, H-4.5);
  ...
  for(let i=0;i<capturedSlots.length;i++){
    ...
    const rt=vehicle.reg||""; doc.text(rt,W-margin-doc.getTextWidth(rt),14);
...
const SCREEN={CONNECT:"connect",INSPECT:"inspect",SENDING:"sending",DONE:"done"};
...
function App(){
  ...
  const signIn=()=>{
    if(!window.google){setError("Loading Google... please wait 2 seconds and try again.");return;}
    const tc=window.google.accounts.oauth2.initTokenClient({
      ...
      callback:async(resp)=>{
        ...
        }catch{setError("Could not fetch profile. Try again.");}
      },
    });
  ...
  const handleSubmit=async()=>{
    ...
    try{
      ...
      const doc=await buildPDF({vehicle,photos,inspectorName:gmailUser.name,inspectorEmail:gmailUser.email,generatedAt});
      ...
      const bodyText=[
        ...
        "","PDF report attached: "+pdfFilename,"","---","Sent via UniInspect",
      ].join("\n");
      ...
      <div style={S.center}>
        <div style={S.centerCard}>
          ...
          <div style={S.featureBox}>
            {["Capture timestamped vehicle photos","Auto-generate PDF inspection report","Send PDF directly to recipient's email","Report also saved on your device"].map(f=>(
          ...
          <p style={{fontSize:11,color:"#94a3b8",marginTop:12}}>Your Gmail is used to send inspection reports</p>
        </div>
      </div>
    </div>
  );
    ...
    <div style={S.page}>
      <div style={S.center}>
        <div style={S.centerCard}>
          ...
          <h2 style={S.bigTitle}>Please wait…</h2>
    ...
    <div style={S.page}>
      <div style={S.center}>
        <div style={S.centerCard}>
          ...
          <p style={S.subTitle}>PDF delivered to</p>
          ...
          <p style={{fontSize:12,color:"#64748b",marginBottom:20}}>PDF also downloaded to your device.</p>
          <button style={S.btn} onClick={()=>{setPhotos({});setVehicle({reg:"",make:"",owner:"",policy:""});setToEmail("");setSendStatus("");setScreen(SCREEN.INSPECT);}}>
  ...
  return(
    <div style={S.page}>
      <div style={S.header}>
        ...
        <div style={S.fromBadge}>Sending from: {gmailUser?.email}</div>
      </div>
      ...
      <div style={S.body}>
        ...
        <div style={S.card}>
          ...
          <div style={{marginBottom:0}}>
            <label style={S.label}>Recipient Email Address *</label>
            <input style={S.input} type="email" placeholder="underwriter@insuranceco.com" value={toEmail} onChange={e=>setToEmail(e.target.value)}/>
          </div>
        </div>
...
const S={
  ...
  fromBadge   :{fontSize:11,color:"#475569",marginTop:8},
