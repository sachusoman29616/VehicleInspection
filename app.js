...
function getGPS() {
  return new Promise(function(resolve) {
    ...
    // 5 second timeout — don't block photo completion
    var timer = setTimeout(function(){ resolve(null); }, 5000);
    navigator.geolocation.getCurrentPosition(
      function(pos) {
        clearTimeout(timer);
        resolve({lat:pos.coords.latitude.toFixed(6),lng:pos.coords.longitude.toFixed(6),accuracy:Math.round(pos.coords.accuracy)});
      },
...
async function sendViaGmail(to, subject, bodyText, pdfBase64, pdfFilename) {
  ...
  var boundary = "uninspect_"+Date.now();
  var nl = "\r\n";
  var email = [
    "MIME-Version: 1.0",
    "To: "+to,
...
function App(){
  ...
  async function captureFromCamera(){
    ...
    var ts=new Date();
    // Show photo immediately — no processing state
    var resized=await resizeImage(dataUrl);
    var stamped=await stampImage(resized,slot.label,formatTs(ts),null,null);
    setPhotos(function(p){var n=Object.assign({},p);n[slotId]={url:stamped,ts:ts,gps:null,placeName:null,processing:false};return n;});
    // GPS in background — updates stamp when ready
    getGPS().then(async function(photoGps){
      var photoPlace=photoGps?(await getPlaceName(photoGps.lat,photoGps.lng)):null;
      if(photoGps||photoPlace){
        var stamped2=await stampImage(resized,slot.label,formatTs(ts),photoGps,photoPlace);
        setPhotos(function(p){var n=Object.assign({},p);n[slotId]={url:stamped2,ts:ts,gps:photoGps,placeName:photoPlace,processing:false};return n;});
